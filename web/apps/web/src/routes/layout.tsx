...
import { MessageSquare, Briefcase, TrendingUp, Settings, LogOut, BarChart3, Moon, FileDown, BookOpen, Sun, Languages, Swords, Map, History, Filter, TrendingDown, Activity, Bell, Menu, X, PanelLeftClose, PanelLeftOpen, type LucideIcon } from 'lucide-react'
...
function PreferenceControls({ collapsed }: { collapsed?: boolean }) {
  ...
  if (collapsed) {
    return (
      <div className="mb-1 flex flex-col items-center gap-1 px-1">
        <button
          type="button"
          onClick={toggleTheme}
        ...
        <button
          ...
          className="flex h-8 w-8 items-center justify-center rounded-lg border border-border text-xs text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
        >
          <Languages size={14} />
        </button>
      </div>
...
function SidebarFooter({ email, onLogout, collapsed }: { email: string; onLogout: () => void; collapsed?: boolean }) {
  const { t } = usePreferences()

  return (
    <div className="border-t border-border p-3">
      <PreferenceControls collapsed={collapsed} />
      {!collapsed && <div className="mb-2 truncate px-3 text-[11px] text-muted-foreground">{email}</div>}
      <button
        onClick={onLogout}
        title={collapsed ? t('action.logout') : undefined}
        className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-muted-foreground transition-colors hover:bg-muted hover:text-foreground ${collapsed ? 'justify-center px-0' : ''}`}
      >
        <LogOut size={15} />
...
function SidebarNav({ pathname, hash, email, onLogout, collapsed, onToggleCollapse }: {
  pathname: string; hash: string; email: string; onLogout: () => void; collapsed: boolean; onToggleCollapse: () => void
}) {
  const { t } = usePreferences()
  return (
    <>
      <div className={collapsed ? 'flex flex-col items-center px-0 py-5' : 'px-5 py-5'}>
        <h2 className="bg-gradient-to-r from-primary to-cyan-500 bg-clip-text text-xl font-bold tracking-tight text-transparent">
          W
        </h2>
        {!collapsed && <p className="mt-0.5 text-[11px] text-muted-foreground">{t('app.subtitle')}</p>}
      </div>
      <nav className={collapsed ? 'flex-1 space-y-0.5 px-1' : 'flex-1 space-y-0.5 px-3'}>
      ...
      <SidebarFooter email={email} onLogout={onLogout} collapsed={collapsed} />
    </>
  )
}
  ...
  return (
    <>
      ...
      >
        ...
        <SidebarNav pathname={pathname} hash={hash} email={email} onLogout={onLogout} collapsed={false} onToggleCollapse={() => {}} />
      </aside>
    </>
  )
}
...
export function AppLayout() {
  ...
  return (
    <div className="flex h-screen">
      <aside className={`hidden lg:flex ${collapsed ? 'w-14' : 'w-56'} flex-col border-r border-border bg-sidebar transition-all duration-200`}>
        <SidebarNav pathname={pathname} hash={hash} email={email} onLogout={handleLogout} collapsed={collapsed} onToggleCollapse={toggleCollapse} />
      </aside>
